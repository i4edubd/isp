...
# ✅ Developer Checklist for ISP Bills Refactor
...
- [ ] Replace legacy asset pipeline with **Vite** (`vite.config.js`).
- [ ] Configure hot module replacement (HMR) for local dev.
- [ ] Ensure Tailwind purge paths include `resources/views` and `resources/js`.
- [ ] Refactor Blade templates to use Tailwind utility classes.
- [ ] Test Chart.js, Alpine.js, Axios integrations under Vite bundling.

---

## 4. Testing & Quality Assurance
- [ ] Implement **PestPHP** or PHPUnit for unit and feature tests.
- [ ] Add frontend tests with **Vitest** or **Jest**.
- [ ] Run static analysis with **PHPStan** and **Larastan**.
...
- [ ] Configure Laravel Sanctum/Passport for API authentication.
- [ ] Validate WebAuthn integration for passwordless login.
- [ ] Optimize queries with Eloquent scopes and indexes.
- [ ] Run `php artisan optimize` and cache configs/routes/views.

---

## 6. Documentation
- [ ] Update developer onboarding guide with new stack requirements.
- [ ] Document Vite + Tailwind build process.
- [ ] Provide migration notes for Laravel 12 changes.
- [ ] Maintain Markdown checklists for each module (auth, billing, router integration).
