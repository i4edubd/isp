...
use App\Jobs\subOperatorDeleteJob;
use App\Models\operator;
use App\Models\pgsql_activity_log;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Cache;

class SubOperatorDeleteController extends Controller
{
    /**
    ...
    public function create(Request $request, operator $operator)
    {
        ...
        if ($operator->role == 'sub_operator' && $operator->gid == $group_admin->id) {
            $delete = 1;
        }

        if ($delete == 0) {
            return redirect()->route('sub_operators.index')->with('error', 'Not Authorized!');
        }

        $key = 'sub_operator_destroy_' . $operator->id;
        $ttl = 1800;
        ...
        return view('admins.operator.sub_operator-destroy-create', [
    ...
    public function store(Request $request, operator $operator)
    {
        ...
        if ($group_admin->two_factor_recovery_codes !== $request->code) {
            return redirect()->route('sub_operators.index')->with('error', 'Invalid Code!');
        }
        ...
        pgsql_activity_log::create([
            ...
            'topic' => 'destroy_sub_operator',
            'year' => date(config('app.year_format')),
            'month' => date(config('app.month_format')),
            'week' => date(config('app.week_format')),
            'log' => $request->user()->name . ' has deleted sub_operator: ' . $operator->name,
        ]);

        $operator->deleting = 1;
        $operator->save();

        subOperatorDeleteJob::dispatch($operator)
            ->onConnection('database')
            ->onQueue('default');

        return redirect()->route('sub_operators.index')->with('success', 'Job is processing');
    }
}
