...
@section('content')
    <div class="card">
        ...
        <div class="card-body text-center">
            ...
            <p class="card-text">Amount: {{ $package->price }} {{ getCurrency($operator->id) }}</p>
        ...
        $(document).ready(function() {
            ...
            bKash.init({
                ...
                createRequest: function(request) {
                    $.ajax({
                        method: 'GET',
                        url: "/bkash/customer_payment/" + customer_payment + "/create?token=" +
                            token,
                ...
                executeRequestOnAuthorization: function() {
                    $.ajax({
                        method: 'GET',
                        url: "/bkash/customer_payment/" + customer_payment + "/execute?token=" +
                            token,
                        success: function(data) {
                            data = JSON.parse(data);
                            if (data && data.paymentID != null) {
                                if (data.transactionStatus == 'Initiated') {
                                    window.location.href = "/bkash/customer_payments/" +
                                        customer_payment;
                                } else {
                                    window.location.href = "/bkash/customer_payment/" +
