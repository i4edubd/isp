...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Edit Device Monitor</h3>
        </div>
        <form action="{{ route('device-monitors.update', $deviceMonitor) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                @if ($errors->any())
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="device_name" id="device_name" class="form-control" 
                                   value="{{ old('device_name', $deviceMonitor->device_name) }}" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="device_type" id="device_type" class="form-control" required>
                                <option value="ap" {{ old('device_type', $deviceMonitor->device_type) == 'ap' ? 'selected' : '' }}>Access Point</option>
                                <option value="host" {{ old('device_type', $deviceMonitor->device_type) == 'host' ? 'selected' : '' }}>Host Server</option>
                                <option value="mikrotik" {{ old('device_type', $deviceMonitor->device_type) == 'mikrotik' ? 'selected' : '' }}>Mikrotik</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="ip_address" id="ip_address" class="form-control" 
                                   value="{{ old('ip_address', $deviceMonitor->ip_address) }}" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="monitor_method" id="monitor_method" class="form-control" required>
                                <option value="ping" {{ old('monitor_method', $deviceMonitor->monitor_method) == 'ping' ? 'selected' : '' }}>Ping</option>
                                <option value="snmp" {{ old('monitor_method', $deviceMonitor->monitor_method) == 'snmp' ? 'selected' : '' }}>SNMP</option>
                            </select>
                        </div>
                    </div>
                </div>

                <div class="row" id="snmp_fields">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="port">SNMP Port</label>
                            <input type="number" name="port" id="port" class="form-control" 
                                   value="{{ old('port', $deviceMonitor->port ?? 161) }}" min="1" max="65535">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="snmp_community" id="snmp_community" class="form-control" 
                                   value="{{ old('snmp_community', $deviceMonitor->snmp_community ?? 'public') }}">
                        </div>
                    </div>
                </div>
            </div>
