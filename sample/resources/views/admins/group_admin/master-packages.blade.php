                        ...
                        @foreach ($master_packages->where('connection_type', 'PPPoE')->sortBy('name') as $master_package)
                            <tr>
                                ...
                                <td>
                                    {{ $master_package->name }}
                                    <br>
                                    <span class="font-italic"> Profile: {{ $master_package->pppoe_profile->name }} </span>
                                    <br>
                                    <span class="font-italic">
                                        IPv4: {{ $master_package->pppoe_profile->ipv4pool->name }} ::
                                        {{ long2ip($master_package->pppoe_profile->ipv4pool->subnet) .
                                            '/' .
                                            $master_package->pppoe_profile->ipv4pool->mask }}
                                    </span>
                                    <br>
                                    <span class="font-italic"> IPv6: {{ $master_package->pppoe_profile->ipv6pool->name }} ::
                                        {{ $master_package->pppoe_profile->ipv6pool->prefix }}
                                    </span>
                                </td>
                                <td>
                                    {{ $master_package->rate_limit }} {{ $master_package->readable_rate_unit }}
                                    <br>
                                    Speed Controller : {{ $master_package->speed_controller }}
                                </td>
                                <td>{{ $master_package->volume_limit }} {{ $master_package->volume_unit }}</td>
                                <td>{{ $master_package->validity }} {{ $master_package->validity_unit }}</td>
            ...
            <div class="table-responsive-sm">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @foreach ($master_packages->where('connection_type', 'Other')->sortBy('name') as $master_package)
                            <tr>
                                <td>{{ $master_package->id }}</td>
                                <td>{{ $master_package->name }}</td>
                                <td>{{ $master_package->operator_price }}</td>
                                <td>{{ $master_package->price }}</td>
                                <td>{{ $master_package->validity }} {{ $master_package->validity_unit }}</td>
                                <td>{{ $master_package->customer_count }}</td>
                                <td>
                                    @foreach ($master_package->operators as $operator)
