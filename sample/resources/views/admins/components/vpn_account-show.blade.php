...
<div class="card card-outline card-primary">
    ...
    <div class="card-body">
        ...
        <br>
        <code>
        add comment="vpn-client" connect-to="{{ $vpn_server->nasname }}:{{ $vpn_server->sstp_port }}" disabled=no name="sstp-{{ $vpn_account->id }}" password="{{ $vpn_account->password }}" profile="vpn-profile" user="{{ $vpn_account->username }}"
    </code>
        <hr>
        <code>
            /ip firewall nat
