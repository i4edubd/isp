...
@section('content')
                ...
                <select name="operator_id" id="operator_id" class="form-control">
                    ...
                    @foreach (Auth::user()->operators->where('role', '!=', 'manager') as $operator)
                        <option value="{{ $operator->id }}">
                            {{ $operator->id }} :: {{ $operator->name }} :: {{ $operator->readableRole }}
                        </option>
                    @endforeach
                </select>
            </div>
    ...
    @can('downloadPayments')
        <ul class="nav justify-content-end">
            ...
            @foreach (Auth::user()->payment_gateways as $payment_gateway)
                <li class="nav-item">
                    <a class="nav-link"
                        href="{{ route('payment_gateways.customer_payments.index', ['payment_gateway' => $payment_gateway->id]) }}">
                        <i class="fas fa-eye"></i>
                        {{ $payment_gateway->payment_method }} Payments
                    </a>
                </li>
            @endforeach
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <nav class="navbar justify-content-end">
                ...
                <a href="#" class="mr-4 btn btn-outline-info" data-toggle="tooltip" data-placement="bottom"
                    title="Customize Payments View"
                    onclick="showViewCustomizeOption('{{ route('customer-payment-breakdown.create') }}')">
                                ...
                                @if (Auth::user()->can('update', $payment) || Auth::user()->can('delete', $payment))
                                    <div class="btn-group" role="group">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="btnGroupActionsOnCustomer">
                                            ...
                                            @can('view', $payment)
                                                <a class="dropdown-item"
                                                    href="{{ route('customer_payments.voucher.create', ['customer_payment' => $payment]) }}">
                                                    Download Voucher
                                                </a>
                                            @endcan
                                            {{-- --}}
...
@section('pageJs')
    <script>
        ...
        function showViewCustomizeOption(url) {
